...
export function Box({
  ...
  const { node: parent, dirtyId } = useContext(boxContext)
  ...
  }, [node, parent, flexProps, centerAnchor, dirtyId, registerBox, unregisterBox])

  // We need to reflow if props change
  useLayoutEffect(() => {
    reflow()
  }, [children, flexProps, dirtyId, reflow])
  ...
  const sharedBoxContext = useMemo(() => ({ node, size, dirtyId }), [node, size, dirtyId])
